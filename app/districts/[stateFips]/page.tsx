...
import { getStateByFips, isStateFips, US_STATES } from "@/lib/us-states";

// Every state page is static: seats come from the deterministic local
// apportionment sequence, so no request-time data is needed.
export const dynamicParams = false;

export function generateStaticParams() {
  return US_STATES.map((state) => ({ stateFips: state.fips }));
}
...
export default async function StateDetailPage({
  ...
  const seats = getSeatCountsAfter(cap)[stateFips] ?? 0;
