...
- Phase 4E Nemotron diagnostic retry in `eval/phase4_capability_retry.py`: the first capability aggregate remains immutable while a tracked zero-spend plan and content-free source proof bind one byte-equivalent replay of the failed Nemotron evidence-extractor request; the retry is the only authorized call, uses `retry_reserve`, caps pre-call authorization at 7,200 microusd, permits no fallback or suffix continuation, preserves the full candidate roster, and cannot reject or select a model; paid execution remains blocked pending independent review and merge, a fresh catalog preflight, and a new manual approval lasting at most 30 minutes; before loading credentials or touching the network, the single-use local runner atomically claims the retry-plan hash under `private_runs`, with the authorization-bundle hash and hashed private-relative state-output path in the claim body, and that claim survives crashes or ambiguous delivery so changing/deleting the requested state-output path cannot authorize a resend
