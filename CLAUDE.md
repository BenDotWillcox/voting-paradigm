...
**Next, in order:**
1. Manually verify Together privacy toggles, place a project-scoped key only in
   the ignored local environment, and run the zero-spend public-source plus
   authenticated model-catalog preflight
2. Build the exact qualification tokenizer manifest and wave-aware held-out
   calibration with per-call exact counting, then freeze a minimum headroom
   policy and construct the capability-preflight authorization
3. After separate spend approval, run the public-development qualification
   within the USD 4 cap
4. Freeze the selected model/deployment, prompts, seeds, remaining policy
